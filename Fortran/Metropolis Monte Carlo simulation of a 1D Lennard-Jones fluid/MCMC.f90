...
program LJFluid
    ...
    halfL=L/2.0d0   										!calculating half length
    a=[0.95d0,1.12d0]   									!for different values of a (lattice constant)
	nDisc=3000										!number of initial monte carlo cycles to be discarded
    ...
    do k=1,2											!for selecting value of a
        do i=0,n-1	
	        x(i+1)=i*a(k)									!setting the initial positions of the particles
	    enddo
	    sumU=0.0d0
        do i=1,nmc      									!MC cycle
            do p=1,n    									!generating random positions
                r=rand()
                j=int(n*r)+1    								!taking random particle
                if (j>n)j=n     
                
                r=rand()
                delX=2.0d0*(r-0.5d0)*delta
                xj=x(j)             								!taking jth partical position
                xTest=xj+delX       								!changing randomly choosen particle's position
                ...
                if (uNew<=uOld) then    							!applying transition probability condition
                    x(j)=xTest
                else
                    delU=uNew-uOld
...
subroutine Energy1(j,xj,u1)
    ...
    u1=0.0d0
    do i=1,n
        if(i==j) cycle  									!if the condition is satisfied, then discard cycle statement and continue below calculation for that i
        
        xji=xj-x(i)             								!calculating distance between 2 particles
