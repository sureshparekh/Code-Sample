...
program Ising2D
    ...
    do i=1,n	!initialization
        do j=1,n 
            r=rand()
            S(i,j)=(-1)**nint(r)					!there is 3 methods, one is to randomly orient spins
        enddo
    enddo
    ...
    do i=1,nmc								!monte-carlo cycle
        do j=1,n2							!selecting random particle and flip it's spin 
            ...
            !uOld=u1
            !uNew=-uOld						!particle's new energy is -ve of the old energy
...
subroutine applyPBC()
    ...
    S(:,0)=S(:,n)							!copy nth column to 0th column
    S(:,n+1)=S(:,1)							!copy 0th column to (n+1)th column
    return
...
subroutine TotEnergy(uTot)
    ...
    do i=1,n
        do j=1,n
        uTot=uTot-S(i,j)*(S(i+1,j)+S(i,j+1))				!adding each pair energy
        enddo
    enddo
    return
